...
export const register = async (req, res) => {
  try {
    if (!req.user) {
      ...
      return; //Por las dudas, que salga después de mandar la respuesta.
    } else {
      //res.status(201).send("Usuario creado correctamente."); //200 es OK, 201 es Created.
      res.status(201).redirect("/viewLogin");
...
export const login = async (req, res) => {
  try {
    ...
    //Ahora que ya sé que es un usuario válido, voy a generarle un token.
